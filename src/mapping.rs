...
mod tests {
    ...
    fn transform_f32_to_u32() {
        let tests = vec![32f32,85.2934f32,8393.42f32,-53f32,
                         -0f32, 0f32, std::f32::INFINITY, std::f32::NAN];
        for val in tests {
            if val.is_nan(){
    ...
    fn transform_f64_to_u64() {
        let tests = vec![32.3352,85.2934,8393.42,0.0, -0.0,
                         std::f64::INFINITY, std::f64::NAN];
        for val in tests {
            if val.is_nan() {
    ...
    fn transform_u32_to_f32() {
        let tests = vec![32u32,85u32,293u32,83u32,
                         9342u32,1u32,0u32,984744474u32];
        for val in tests {
    ...
    fn transform_u64_to_f64() {
        let tests = vec![32u64,3352u64,8u64,529u64,
                         34u64,83u64,93u64,42u64,984742123298754474u64];
        for val in tests {
